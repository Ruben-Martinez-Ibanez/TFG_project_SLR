...
data_section = [190, 240]
station_code: int = 7839

# CRD_file: str = 'ajisai_20231204.fr2'
...
plt.plot(day_t_part_full, residuos_mil_scale_full, '.')
...
y = []
x = []
residuos_mil_sin_trend = []
media_res = 0
for i in range(len(day_t_part_full)):
    if day_t_part_full[i]<50:
        x.append(day_t_part_full[i])
        y.append(1.5*np.sin(0.32*x[i]-15))
        residuos_mil_sin_trend.append(residuos_mil_scale_full[i]-y[i])
        media_res += residuos_mil_sin_trend[i]
media_res = media_res/len(residuos_mil_sin_trend)
...
plt.ylim(-6, 6)
plt.title(f'{satelitte} - {fecha_datos}',fontsize=37)
plt.savefig(f'7. Sección de residuos en Escala milimétrica ajuste - {satelitte} - {fecha_datos}.jpg', dpi=500.0)
plt.show()
...
for i in range(len(residuos_mil_sin_trend)):
    sumatorio_cuadrado += (residuos_mil_sin_trend[i]-media_res)**2
    if residuos_mil_sin_trend[i] < -4 or residuos_mil_sin_trend[i] > 4:
        res_ext.append(residuos_mil_sin_trend[i])
        t_ext.append(x[i])
sigma = np.sqrt(sumatorio_cuadrado/len(residuos_mil_sin_trend))
print(f'Media = {media_res}')
print(f'Sigma = {sigma}')
...
plt.plot(x, residuos_mil_sin_trend, '.')
plt.plot(t_ext, res_ext, 'm.')
plt.axhline(media_res, xmin = 0, xmax = 50, color = 'r', linewidth= 7.0)
plt.axhline(media_res+f_sigma, xmin = 0, xmax = 50, color = 'g', linewidth= 7.0)
plt.axhline(media_res-f_sigma, xmin = 0, xmax = 50, color = 'g', linewidth= 7.0)
plt.xlabel('Tiempo [s]',fontsize=45)
plt.ylabel('Residuos [mm]',fontsize=45)
plt.yticks(fontsize=30)
plt.xticks(fontsize=30)
plt.title(f'{satelitte} - {fecha_datos}',fontsize=37)
plt.savefig(f'8. Sección de residuos en Escala milimétrica detrend - {satelitte} - {fecha_datos}.jpg', dpi=500.0)
plt.show()



residuos_mil_corregido = []
day_t_mil_corregido = []
for i in range(len(residuos_mil_sin_trend)):
    if residuos_mil_sin_trend[i] < media_res+f_sigma:
        if residuos_mil_sin_trend[i] > media_res-f_sigma:
            residuos_mil_corregido.append(residuos_mil_sin_trend[i]+y[i])
            day_t_mil_corregido.append(x[i])
            
plt.figure(figsize=(22, 15))
plt.plot(day_t_mil_corregido, residuos_mil_corregido, '.')
plt.xlabel('Tiempo [s]',fontsize=45)
plt.ylabel('Residuos [mm]',fontsize=45)
plt.yticks(fontsize=30)
plt.xticks(fontsize=30)
plt.title(f'{satelitte} - {fecha_datos}',fontsize=37)
plt.savefig(f'9. Sección de residuos en Escala milimétrica Correccion - {satelitte} - {fecha_datos}.jpg', dpi=500.0)
plt.show()
...
w = np.linspace(0.0001, 2, 10000) #frequencies
a_lomb = lombscargle(day_t_mil_corregido, residuos_mil_corregido, w, normalize=True)

plt.plot(w, a_lomb)
plt.title(f'{satelitte} - {fecha_datos}')
plt.xlabel('ω [rad/s]')
plt.ylabel('Amplitud normalizada')
plt.savefig(f'10. Algoritmo de Lomb Escala Milimétrica - {satelitte} - {fecha_datos}.jpg', dpi=500.0)
plt.show()
